...
data Careu = C [Piesa] deriving (Show,Eq)
type Tabla = [Careu]
...
pieseCount :: [Piesa] -> (Int, Int)
pieseCount [] = (0,0)
pieseCount c = let pieseOne = length $ filter (==One) c in
               let pieseTwo = length $ filter (==Two) c in
               (pieseOne, pieseTwo)

validTabla :: Tabla -> Bool
validTabla [(C c1), (C c2), (C c3)]
   | (length c1) /= 8 || (length c2) /= 8 || (length c3) /= 8 = False
   | otherwise = let pieseOne = sum [fst $ pieseCount x | x <- [c1,c2,c3]] in
                 let pieseTwo = sum [snd $ pieseCount x | x <- [c1,c2,c3]] in
                 pieseOne <= 9 && pieseTwo <= 9
validTabla _ = False
...
changeNthPos :: Int -> a -> [a] -> [a]
changeNthPos _ _ [] = []
changeNthPos n val (h:t)
   | n == 0 = val:t
   | otherwise = h:(changeNthPos (n - 1) val t)
...
instance Monad EitherWriter where
   return a = EW (Right (a, ""))
   EW (Left s) >>= amb = EW (Left s)
   EW (Right (a, s)) >>= amb = amb a
