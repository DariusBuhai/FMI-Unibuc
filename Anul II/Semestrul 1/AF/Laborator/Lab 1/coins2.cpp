  ...
  long long ComputeSCC() {
    ...
    for (auto i : order) {
      if (!vis[i]) {
        ...
        curr_comp.clear();
        DFS2(i, scc_count);
        // ^ curr_comp imi va tine componenta tare conexa

        long long curr_max_coins = 0;
        for (auto node : curr_comp) {
          for (auto nei : graph[node]) {
            // functional echivalenta cu parcurgerea 
            // pe "condensation graph"
            
            // if-ul de mai jos e optional.
            if (comp[node] != comp[nei]) {
              
              // sigur componenta vecinului e in dreapta.
        ...
        // actualizam # monezi cu valorile din ctc curenta
        for (auto node : curr_comp)
          curr_max_coins += coins[node];
        
        // actualizam raspunsul in toate nodurile
        for (auto node : curr_comp)
