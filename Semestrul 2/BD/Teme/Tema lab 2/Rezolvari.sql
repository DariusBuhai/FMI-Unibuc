...
--8
select e.last_name as "angajat", c.cust_name as "client" from orders_tbl o join employee_tbl e on (o.sales_rep = e.emp_id) join customer_tbl c on (o.cust_id = c.cust_id);
--9
select prod_desc from orders_tbl o join products_tbl using (prod_id) where o.sales_rep in 
(select sales_rep from orders_tbl join products_tbl using (prod_id) where length(prod_desc)>2 and lower(prod_desc) like '% p%');
...
--13
select first_name, salary, o."last_date" from employee_tbl join employee_pay_tbl using (emp_id), (select max(ord_date) as "last_date", sales_rep from orders_tbl group by sales_rep) o
where o.sales_rep = emp_id;
--14
...
--18
select cust_name, cust_city, cust_zip from customer_tbl c where cust_zip like '[0-9]%'
and (select count(ord_num) from orders_tbl where cust_id = c.cust_id) = 0;
...
--19
select emp_id, last_name, city, c.cust_id, c.cust_name, c.cust_city from employee_tbl, 
(select cust_id, cust_name, cust_city from customer_tbl) c where c.cust_city != city;
--20
...
WHERE SALARY = '40000');
--22
select first_name, pay_rate from employee_tbl join employee_pay_tbl using (emp_id)
where pay_rate > (select max(pay_rate) from employee_tbl join employee_pay_tbl using (emp_id) where lower(last_name) like '%ll%');
