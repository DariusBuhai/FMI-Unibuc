...
class Model(torch.nn.Module):
    ...
    def forward(self, x):
        x = normalize(x, p=2, dim=0)
        return self.layers(x)


class Dataset(torch.utils.data.Dataset):
    def __init__(self, features: pd.DataFrame, prediction_mode: bool = False):
        self.samples = features.loc[:, ~features.columns.isin(
            ['Unnamed: 0', 'likes', 'likes/followers', 'likes/mean', 'comments', 'engagement',
             'mean'])].to_numpy().astype(
            float)
        if prediction_mode:
            self.labels = [0 for _ in range(features.shape[0])]
            return
        self.labels = features['likes/mean'].to_numpy().astype(float)
...
class Regressor:
    ...
    def predict(self, inp):
        self.model.eval()
        features = pd.DataFrame([inp])
        dl = DataLoader(Dataset(features, prediction_mode=True), batch_size=1, shuffle=False)
        for i, data in enumerate(dl, 0):
            inputs, _ = data
            output = self.model(inputs.float())
            return output[0].detach().numpy()[0]
